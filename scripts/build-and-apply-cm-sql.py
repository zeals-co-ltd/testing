...
# kubectl create cm {file.name} --from-file {file.path} -n load-test-sql

def get_cm_dict() -> dict:
    path = "./src/load_test_event_slayers/sql-tests"
...
def create_cm(cm_dict: dict):
    ...
    for k,v in cm_dict.items():
        ...
        # command = f"kubectl create cm {k} --from-file={os.getcwd()}/src/load_test_event_slayers/sql-tests/{v} -n load-test-sql"
        subprocess.run(["kubectl", "create", "cm", k, f"--from-file={os.getcwd()}/src/load_test_event_slayers/sql-tests/{v}", "-n", "load-test-sql"])
...
spec:
  ...
  script:
    configMap:'''
        ...
        file += f"      file: {v}\n"
        file += f'''  runner:
      image: gcr.io/zeals-cr/xk6-sql:latest
      env:
